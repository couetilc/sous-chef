...
class Command(BaseCommand):
    help = 'Load recipe turkey scores and notes from CSV into the database'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help='Path to the CSV file (default: /scraping/production/turkey_scores.csv)',
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Reading turkey scores from: {csv_path}')
        ...
        try:
            with open(csv_path, 'r', encoding='utf-8') as csvfile:
                ...
                required_columns = ['title', 'turkey_score', 'turkey_notes']
                ...
                for row_num, row in enumerate(reader, start=2):  # Start at 2 to account for header
                    ...
                    score_str = row.get('turkey_score', '').strip()
                    notes = row.get('turkey_notes', '').strip()
        ...
        self.stdout.write(f'Found {len(recipe_scores)} turkey scores in CSV')

        # Dry run check
        if options['dry_run']:
            self.stdout.write(self.style.WARNING('\n=== DRY RUN - No database changes made ==='))
            self.stdout.write(f'Sample turkey scores (first 10):')
                ...
                for title in batch_titles:
                    if title in recipes_by_title:
                        ...
                        for recipe in recipe_list:
                            recipe.turkey_score = score_data['score']
                            recipe.turkey_notes = score_data['notes']
