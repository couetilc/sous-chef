...
with psycopg.connect(conn_info, row_factory = psycopg.rows.dict_row) as conn:
    with conn.cursor() as cur:
        ...
        cur.execute("CREATE TABLE scraped_prices (id serial PRIMARY KEY, ingredient_name TEXT, quantity_other TEXT, price TEXT)")
        with path.open(newline="", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                cur.execute(
                    "INSERT INTO scraped_prices (ingredient_name, quantity_other, price) VALUES (%s,%s,%s)",
                    (row["ingredient_name"], row["quantity_other"], row["price"])
                )
